 ...
 public:
  ...
  void regress() {
    ...
    for (int k = 0; k < MAX_ITER; ++k) {
      ...
      std::nth_element(r.begin(), middle, r.end()); // todo: replace with nth_element.
...
double SumAbsBuffer(const SampleBuffer& buffer,
		    int len) {
  double ret = 0;
  for (int i = 0; i < len; ++i) {
...
double MaxBuffer(const SampleBuffer& buffer,
		    int len) {
  double ret = 0;
  for (int i = 0; i < len; ++i) {
...
double ZeroCrossingsBuffer(const SampleBuffer& buffer,
			   int len) {
  double ret = 0;
...
double SpectralFlatnessBuffer(const SampleBuffer& buffer,
			      int len) {
...
AudioSummary SummarizeTrack(const WaveTrack* track,
  ...
  AudioSummary ret;
  ret.time_per_summarized_sample_millis = options.time_per_summarized_sample_millis;
  const int buff_size = track->GetRate() * ret.time_per_summarized_sample_millis / 1000 + 2;
  ...
  SampleBuffer buffer(buff_size, floatSample);
   ...
   int elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>
                             (std::chrono::system_clock::now() - time_start).count();
   std::cout << "Summarizing " << track->GetName() << " took " << elapsed_ms << "ms" << std::endl;
   return ret;
}

double CorrelationNormalization(double match_fraction) {
  return 1;
}
...
float GetModuloSize(const std::vector<float>& v,
		    int i) {
  i %= v.size();
  if (i < 0) i += v.size();
  wxASSERT(i >= 0);
...
bool IsWithinMatchPercentage(int match_position,
			     int min_match_percent,
			     int reference_size,
			     int other_size,
			     int total_size) {
  ...
  return interval_other.Size() /
    std::min(reference_size, other_size) * 100 >= min_match_percent;
}
...
int FindShift(const std::vector<float>& normalized_cross_correlation,
  ...
  case ShiftFindingMethod::kMaxPeakToAveragePowerRatio: {
    ...
    const int kWindowHalfSize = kTimeWindowSec /
      options.time_per_summarized_sample_millis * 1000 / 2;
    double sum_squares = 0;
    std::vector<float> sum_squares_debug;
    for (int i = -kWindowHalfSize - 1; i < kWindowHalfSize; ++i) {
      sum_squares += std::pow(GetModuloSize(normalized_cross_correlation, i), 2);
    }
    ...
    for (int i = -kWindowHalfSize, j = 0, k = kWindowHalfSize;
	 j < normalized_cross_correlation.size();
	 i++, j++, k++) {
      // Remove [i-1], add [k]
      sum_squares -= std::pow(GetModuloSize(normalized_cross_correlation, i - 1), 2);
      sum_squares += std::pow(GetModuloSize(normalized_cross_correlation, k), 2);

      double sum_squares_naive = 0;
      for (int m = i; m <= k; ++m) {
	sum_squares_naive += std::pow(GetModuloSize(normalized_cross_correlation, m), 2);
      }
      // TODO: compare sum_squares and sum_squares_naive, and check
      // for numerical drift.
      sum_squares_debug.push_back(sum_squares);
      if (sum_squares <= 0 ||
	  !IsWithinMatchPercentage(j, kMinMatchPercent, reference_size, other_size,
				   normalized_cross_correlation.size())) {
	papr[j] = 0;
      } else {
	papr[j] = std::pow(normalized_cross_correlation[j], 2) /
	  sum_squares *
	  // The goal of this is to also take into account the
	  // amplitude of the peaks, not just their peakiness.
	  std::pow(normalized_cross_correlation[j], options.correlation_normalization_power);
      }
    }
    if (print_debug) {
  ...
  wxFAIL_MSG("Unknown shift finding method " + std::to_string(options.shift_finding_method));
}
...
std::pair<int, double> Align(const AudioSummary& reference,
  ...
  // multiply by 2.
  int rounded_size = 2 * RoundNextPowerOf2(std::max(reference.summarized_amplitudes.size(),
  ...
  std::copy(reference.summarized_amplitudes.begin(),
	    reference.summarized_amplitudes.end(),
	    ref.begin());
  ...
  RealFFT(ref.size(),
	  &ref[0],
	  &reference_fft_real[0],
	  &reference_fft_imag[0]);
  ...
  */
  ...
  for (int i = 0; i < cross_correlation.size(); ++i) {
    ...
    interval_other.Intersect(interval_ref);
    const double match_fraction = interval_other.Size() /
      std::min(reference.summarized_amplitudes.size(),
	       to_align.summarized_amplitudes.size());
    normalized_cross_correlation[i] = cross_correlation[i] *
      CorrelationNormalization(match_fraction);
    normalization_factors[i] = CorrelationNormalization(match_fraction);
  }

  if (print_debug) {
    std::ofstream out_normalized_cross("normalized_cross.csv");
    PrintVector(normalized_cross_correlation,
		"normalized_cross",
		out_normalized_cross);
    std::ofstream out_normalized2_cross("normalized2_cross.csv");
    PrintVector(normalization_factors,
		"normalized2_cross",
		out_normalized2_cross);
    std::cout << "Computing cross correlations took "
	      << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - time_start).count()
	      << "ms" << std::endl;
  }

  const int raw_shift = FindShift(normalized_cross_correlation,
				  options,
  ...
  double time_shift_s = shift * reference.time_per_summarized_sample_millis / 1000;
  wxASSERT(to_align.time_per_summarized_sample_millis ==
	   reference.time_per_summarized_sample_millis);
  if (print_debug) {
    std::cout << "raw shift: " << raw_shift
	      << " Shift: "<< shift << " time shift s: "
	      << time_shift_s << " size: "<< rounded_size << " "
	      << reference.summarized_amplitudes.size()
	      << " " <<  to_align.summarized_amplitudes.size() << std::endl;
...
FindTimeDrift(int offset, // Initial offset from global alignment.
	      const AudioSummary& reference,
	      const AudioSummary& to_align,
	      const AudioAlignOptions& options) {
  ...
  for (int window_start = 0, window_end = window_size ;
    ...
    AudioSummary reference_windowed = reference.Window(reference_window_start, reference_window_end);
    int offset = Align(reference_windowed, to_align_windowed, options, false).first;
    ...
    window_centers.push_back((reference_window_start + reference_window_end) / 2.);
    regression.point((window_start + window_end) / 2.,
		     offset);
  }
  if (offsets.size() < 5) {
    std::cout << "Not enough windows to detect clock drift: "
	      << offsets.size() << " vs: " << 5 << std::endl;
  ...
  double speed_multiplier =
    1 / (1. + regression.f(1) - regression.f(0));
...
std::pair<double, double> AlignTracks(const WaveTrack* reference_track,
				      ...
				      AudioAlignOptions options) {
  std::cout << "Summarizing reference track: "
	    << reference_track->GetName() << " offset: "<< reference_track->GetOffset()
	    << " end time: " << reference_track->GetEndTime() << std::endl;
  std::ofstream out_ref("ref.csv");
  AudioSummary reference_summary = SummarizeTrack(reference_track,
						  options,
						  out_ref);
  std::ofstream out_other("other.csv");
  AudioSummary summary = SummarizeTrack(other_track, options,
					out_other);
  std::pair<int, double> offset = Align(reference_summary, summary, options);

  std::pair<double, double> ret;
  if (options.correct_time_drift) {
    std::pair<double, double> drift = FindTimeDrift(offset.first, reference_summary,
						     summary, options);
  ...
  out_file << "normalized_cross = read.csv('normalized_cross.csv', header=F)" << std::endl;
  out_file << "dev.new(); plot(normalized_cross$V1, type='l')" << std::endl;
  out_file << "normalized2_cross = read.csv('normalized2_cross.csv', header=F)" << std::endl;
  ...
  out_file << "sum_squares = read.csv('sum_squares.csv', header=F)" << std::endl;
  ...
  out_file << "window_centers = read.csv('window_centers.csv', header=F)" << std::endl;
...
std::string GetTransformFromFilename(const std::string& filename) {
  ...
  std::string transform_part = dir_without_time_part.substr(dir_without_time_part.rfind('/') + 1);
  return transform_part + ":" + (time_part == "full" ? time_part : std::string("60s"));
}
...
EffectAudioAlign::EffectAudioAlign(bool bench)
{
  m_Bench = bench;
}
...
{
  return m_Bench ? AUDIOALIGN_BENCHMARK_PLUGIN_SYMBOL : AUDIOALIGN_PLUGIN_SYMBOL;
}
...
EffectType EffectAudioAlign::GetType()
{
   return EffectTypeProcess;
}

bool EffectAudioAlign::GetAutomationParameters(EffectAutomationParameters & parms)
{
   return true;
}

bool EffectAudioAlign::SetAutomationParameters(EffectAutomationParameters & parms)
{
   return true;
}
...
bool EffectAudioAlign::Startup()
{
  return true;
}

bool EffectAudioAlign::Init()
{
   return true;
}
...
bool EffectAudioAlign::Process()
{
  ...
  for (Track* t = iter.First(); t; t = iter.Next()) {
    ...
    std::cout << "Processing selected wave track"
	      << wave_track->GetName() << std::endl;
...
void EffectAudioAlign::ResampleTrack(double time_multiplier,
				     WaveTrack* input) {
  std::cout << "Resampling track: " << input->GetName()
	    << " multiplier: " << time_multiplier << std::endl;
  auto newTrack = mFactory->NewWaveTrack(input->GetSampleFormat(),
						      input->GetRate());
  ...
  sampleCount end = input->TimeToLongSamples(input->GetEndTime());
   ...
   while (samplePos < end) {
      auto blockSize = limitSampleBufferSize(
         input->GetBestBlockSize(samplePos),
         end - samplePos
      );
      input->Get((samplePtr) inBuffer.get(), floatSample, samplePos, blockSize);

      const auto results = resample.Process(time_multiplier,
                                    inBuffer.get(),
                                    blockSize,
      ...
      if (outgen > 0) {
	newTrack->Append((samplePtr)outBuffer.get(), floatSample,
			 outgen);
      }
   ...
   LinearTimeWarper warper { input->GetStartTime(), input->GetStartTime(),
       input->GetEndTime(), input->GetStartTime() + newLength };
...
void EffectAudioAlign::Benchmark(const wxString& dir_str,
  ...
  */
  ...
  for (const auto& c : contents) {
    ...
    bool success = Importer::Get().Import(c,
					  mFactory,
					  newTracks,
					  &tags,
					  errorMessage);
  ...
  std::cout << dir_str << " expected: " << expected_shift << " got: " << offset
	    << " error: " << stats.error
	    << " " << (stats.error < MAX_ERROR_SEC ? "SUCCESS" : "FAILURE") << std::endl;
...
void EffectAudioAlign::RunBenchmarks() {
  ...
  for (auto summary_type : {SummaryType::kSumAbs, SummaryType::kMax,
	//SummaryType::kZeroCrossings,
	//SummaryType::kSpectralFlatness
	}) {
    for (auto shift_finding_method : {//ShiftFindingMethod::kMax,
	...
	for (auto time_per_summarized_sample_millis : {25., 10.}) {
	  ...
	  bench_options.correlation_normalization_power = correlation_normalization_power;
	  bench_options.time_per_summarized_sample_millis = time_per_summarized_sample_millis;
	  std::vector<BenchmarkStats> all_stats;
	  ...
	  for (const auto& s : all_stats) {
	    ...
	    std::cout << (s.success ? "SUCCESS: " : "FAILURE: ") << s.a << " " << s.b
		      << " error: " << s.error << std::endl;
	    //	    std::cout << "TRANSFORM: " << GetTransformFromFilename(s.b) << std::endl;
	  }
	  std::cout << "NUM SUCCESSES: " << num_success << " NUM_FAILURES: "
		    << (all_stats.size() - num_success) << std::endl;
...
bool EffectAudioAlign::TransferDataToWindow()
{

   return true;
}

bool EffectAudioAlign::TransferDataFromWindow()
{
