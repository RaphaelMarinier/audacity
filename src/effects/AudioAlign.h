/**********************************************************************
  ...
  1. Summarize each track by computing a summary value for a sliding
  ...
  2. Compute the cross-correlation between the summaries. This is
...
struct AudioAlignOptions {
  ...
  ShiftFindingMethod shift_finding_method =
...
class EffectAudioAlign final : public Effect {
 ...
 private:
  ...
  void ResampleTrack(double time_multiplier, WaveTrack* input);

  // Benchmark: performs audio alignment of all files in the directory
  // (incl. subdirs, recursively), compares with the "expected" file,
  // and reports stats.
  void Benchmark(const wxString& dir, const AudioAlignOptions& options,
